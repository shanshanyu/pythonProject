'''
用法：
python3 increase_replica.py  hybrid02  hybrid03 canary_topic
第一个是 zk 主机名
第二个是 kafka 主机名
'''
import argparse
...
def get_kafka_broker_id_byshell(zookeeper_host,kafka_host):
    get_broker_id_cmd = f"zookeeper_client -server {zookeeper_host} ls /brokers/ids|grep 'Exiting JVM with code 0' -B 1|head -1|tr -d '[:space:][]'"
    output = ''
    try:
        completed_process = subprocess.run(get_broker_id_cmd, shell=True, text=True, stdout=subprocess.PIPE,
    ...
    if output == '':
        try:
            get_broker_id_cmd = f"kafka-topics --bootstrap-server {kafka_host}:9092 --describe |grep Replicas|awk '{{print $8}}' |tr ',' '\n'|sort -n|uniq|paste -sd,"
            completed_process = subprocess.run(get_broker_id_cmd, shell=True, text=True, stdout=subprocess.PIPE,
                                               stderr=subprocess.PIPE)
...
def increase_partition(topic_name, zookeeper_host, kafka_host):
    ...
    broker_id_list_str = get_kafka_broker_id_byshell(zookeeper_host,kafka_host)
    broker_id_list = broker_id_list_str.split(',')

    reassign_cmd = f'{reassign_script} --bootstrap-server {kafka_host}:9092 --topics-to-move-json-file {reassign_file} ' \
    ...
    reassign_cmd_exec = f'{reassign_script} --bootstrap-server {kafka_host}:9092 --reassignment-json-file {reassign_increase_file} --execute|grep replicas'

    try:
        completed_process = subprocess.run(reassign_cmd_exec, shell=True, text=True, stdout=subprocess.PIPE,
                                           stderr=subprocess.PIPE)
        output = completed_process.stdout
    except Exception as e:
        print(f"执行命令时出错: {str(e)}")
    ...
    print(
        f'{reassign_script} --bootstrap-server {kafka_host}:9092 --reassignment-json-file {reassign_increase_file} --verify')
    ...
    print('可以执行如下命令恢复原状：')
    print(f'{reassign_script} --bootstrap-server {kafka_host}:9092 --reassignment-json-file {restore_file} --execute')
...
if __name__ == '__main__':
    ...
    parser.add_argument('kafka_host', help='kafka host')
    parser.add_argument('topic', help='topic name')
    args = parser.parse_args()
    increase_partition(args.topic, args.zookeeper_host, args.kafka_host)
