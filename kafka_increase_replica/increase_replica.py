...
def increase_partition(topic_name,zookeeper_host):
    ...
    reassign_script = reassign_script if os.path.exists(reassign_script) else '/sensorsdata/main/program/sp/sdp/1.0.0.0-123/kafka/bin/kafka-reassign-partitions.sh'
    ...
    reassign_cmd = f'{reassign_script} --zookeeper {zookeeper_host} --topics-to-move-json-file {reassign_file} --broker-list {broker_id_list_str}  --generate |sed -n "2p"'
    try:
