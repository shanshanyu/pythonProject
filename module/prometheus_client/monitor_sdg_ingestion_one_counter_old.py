...
class MonitorSDGJob:
    ...
    def sigint_handler(self,signalnum,frame):
        self.log_file.close()
        sys.exit(0)

    def running(self,job_dic):
        ...
        while True:
            ...
            for project_id,job_lst in job_dic.items():
                if not job_lst:
                    continue
            ...
            db.close()
            print(time.strftime('%Y%m%d-%H:%M:%S',time.localtime()),'running',file=self.log_file)
            self.log_file.flush()
            time.sleep(60)
