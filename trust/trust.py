...
from env_utils import get_self_ip
import logging
...
MY_DIR = os.path.dirname(os.path.abspath(__file__))  #当前目录
#WORK_DIR = os.path.join(MY_DIR,'..')   #程序所在目录的上层目录
WORK_DIR = MY_DIR
print(WORK_DIR)
...
class Trust(object):
    def __init__(self,conf_file):
        ...
        self.local_program_tar = os.path.join(os.path.dirname(WORK_DIR),self.program_tar_name) #注意 dirname
    ...
    def copy_program_to_hosts(self):
        def copy_from_local(host,counters,lock):
            ...
            if not host == self.ip: #如果host不是本机
                #copy program
                ssh_client.copy_from_local(self.local_program_tar,self.remote_program_tar,timeout=1800)

                #decompress program
                cmd = 'cd {} && tar -zxf {}'.format(os.path.dirname(WORK_DIR),self.remote_program_tar)
                res = ssh_client.run_cmd(cmd,timeout=1800)
                if res['ret'] != 0:
                    raise Exception(res)
